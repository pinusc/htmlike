class @Input
  constructor: (@box, @game) ->
    ...
    this.joystick_l = new Joystick('controller_base', 'controller_ball',
      this.box,
      (baseCenter, diff, dis) ->
        if dis > this.maxJoystickDistance
          offset =
            # dis : maxJoystickDistance = diff.x : offset.x
            x: this.maxJoystickDistance * diff.x / dis
            y: this.maxJoystickDistance * diff.y / dis
          this.ball.cameraOffset.x =
            baseCenter.x + offset.x - this.ball.width / 2
          this.ball.cameraOffset.y =
            baseCenter.y + offset.y - this.ball.height / 2

        else
          offset = diff
        xVelocity =
          offset.x / this.maxJoystickDistance * this.box.m.player.pixelSpeed
        yVelocity =
          offset.y / this.maxJoystickDistance * this.box.m.player.pixelSpeed
        move(xVelocity, yVelocity, this.box.m.player.image.body)
    )

    
    this.joystick_r = new Joystick('controller_base_r', 'controller_ball_r',
      this.box,
      (baseCenter, diff, dis) ->
        if dis < this.maxJoystickDistance
          return
        this.box.m.player.interact(
          direction(this.ball.cameraOffset.x, this.ball.cameraOffset.y,
  ...
  handleDown: () ->
    ...
    if true or not this.game.device.desktop
      x = this.game.input.x
      y = this.game.input.y
...
class Joystick
  constructor: (base, ball, @box, @wrapper) ->
    ...
    # wrapper is a function that gets called every time handle is called,
    # with parameters baseCenter, diff, dis
  ...
  handle: () ->
    ...
    baseCenter =
      ...
      # it is equal to the coordinates of the point that is touched
      # when the joystick appears
      x: this.base.cameraOffset.x + this.base.width / 2
      y: this.base.cameraOffset.y + this.base.height / 2
    diff =
      # this is a point representing the coordinates of the center
      # of the controller ball with respect to the center ball
      # the point being pressed NOW
      x: point.x - baseCenter.x
      y: point.y - baseCenter.y
    # the distance between baseCenter and the point now being pressed
